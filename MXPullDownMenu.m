...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color
{
    ...
    if (self) {
        if (array.count == 1)
        {
            self.frame = CGRectMake(0, 0, 70, 25);
        }
        else
        {
            self.frame = CGRectMake(0, 0, SCREEN_WIDTH/2, 36);
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete
{
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            _tableView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH, tableViewHeight);
        }];
